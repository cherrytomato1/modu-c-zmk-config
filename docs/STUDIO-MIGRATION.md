...
## Fn 레이어의 홈열 방향키와 왼쪽 기호열

레이어2 J/K/L/세미콜론은 ←/↓/↑/→, Q/W/E/R/T는 -/=/[/]/작은따옴표입니다. Mac 기본 전환은 Fn+F, Windows 기본 전환은 Fn+G입니다. 원래 Fn+Q의 Home은 Fn+S로, H/J/K/L의 마우스 버튼은 기호가 빠진 U/I/O/P 위치로 같은 순서(오른쪽/왼쪽/가운데/오른쪽 클릭)로 이동했습니다. 기존 Fn+Y 가운데클릭과 Fn+N 왼쪽클릭도 유지됩니다. 예전 Fn+E/S/D/F 방향키와 Fn+U/I/O/P/세미콜론 기호 위치는 새 배열로 대체됩니다.

Studio에 이미 저장한 키맵이 있으면 새 기본배열이 자동 적용되지 않을 수 있습니다. 필요한 변경을 보관한 후 Restore Stock Settings로 펌웨어 기본값을 불러오거나, 레이어2의 해당 위치를 직접 수정한 다음 Save합니다.

## Fn 레이어 맨 윗줄 F1~F12

레이어2의 맨 윗줄12개(물리 Esc부터 오른쪽 맨끝까지)를 왼쪽부터F1~F12로 배치했습니다. 왼쪽절반은F1~F6, 오른쪽절반은F7~F12입니다. 기본레이어의Esc와 나머지Fn키, 관리레이어5/6부트로더는 그대로입니다. Studio저장키맵을유지하는경우 레이어2맨윗줄을 Key Press F1~F12로설정하고Save합니다.
